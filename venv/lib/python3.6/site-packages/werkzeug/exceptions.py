...
class Unauthorized(HTTPException):
    ...
    Raise if the user_view is not authorized.  Also used if you want to use HTTP
    basic auth.
    """
    code = 401
    description = (
...
class Forbidden(HTTPException):
    ...
    Raise if the user_view doesn't have the permission for the requested resource
...
class TooManyRequests(HTTPException):
    ...
    The server is limiting the rate at which this user_view receives responses, and
    this request exceeds that rate. (The server may use any convenient method
    to identify users and their request rates). The server may include a
    "Retry-After" header to indicate how long the user_view should wait before
    retrying.
    """
    code = 429
    description = (
        'This user_view has exceeded an allotted request count. Try again later.'
    )
