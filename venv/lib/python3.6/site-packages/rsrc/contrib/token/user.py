...
class TokenUser(object):
    """Handle the `token-key` of the user_view.

    `token-key` is token (JWT) related data stored in user_view model.

    Below is the structure of `token-key`:

        token-key               | user_view model
        ----------------------- | ----------
        pk (primary key)        | id
    ...
    def get_key(cls, username, password):
        """Get token-key of the user_view that matches `username` and `password`.
    ...
    def exists(cls, pk, secret):
        """Check if `pk` and `secret` match an existing user_view.
    ...
    def invalidate_key(cls, pk):
        ...
        In recommended implementation, this means:
            + Find an user_view that matches `pk` as his `id`, and change his
            `jwt_secret`, and return True.
            + If `pk` matches no user_view, return False
