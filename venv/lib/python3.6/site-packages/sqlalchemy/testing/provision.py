...
def _oracle_create_db(cfg, eng, ident):
    ...
    # fail otherwise
    with eng.connect() as conn:
        conn.execute("create user_view %s identified by xe" % ident)
        conn.execute("create user_view %s_ts1 identified by xe" % ident)
        conn.execute("create user_view %s_ts2 identified by xe" % ident)
...
def _ora_drop_ignore(conn, dbname):
    try:
        conn.execute("drop user_view %s cascade" % dbname)
