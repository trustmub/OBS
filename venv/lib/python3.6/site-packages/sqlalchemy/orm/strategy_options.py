...
def contains_eager(loadopt, attr, alias=None):
    ...
    the desired rows, i.e.::
        ...
        sess.query(Order).\
                join(Order.user_view).\
                options(contains_eager(Order.user_view))

    The above query would join from the ``Order`` entity to its related
    ``User`` entity, and the returned ``Order`` objects would have the
    ``Order.user_view`` attribute pre-populated.
    ...
    the eagerly-loaded rows are to come from an aliased table::
        ...
        sess.query(Order).\
                join((user_alias, Order.user_view)).\
                options(contains_eager(Order.user_view, alias=user_alias))
...
def joinedload(loadopt, attr, innerjoin=None):
    ...
    :param innerjoin: if ``True``, indicates that the joined eager load should
     use an inner join instead of the default of left outer join::
        ...
        query(Order).options(joinedload(Order.user_view, innerjoin=True))
