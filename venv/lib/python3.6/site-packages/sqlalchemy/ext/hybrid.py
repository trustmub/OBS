...
mapping which relates a ``User`` to a ``SavingsAccount``::
    ...
    class SavingsAccount(Base):
        ...
        id = Column(Integer, primary_key=True)
        user_id = Column(Integer, ForeignKey('user_view.id'), nullable=False)
...
``SavingsAccount`` entry in the list of accounts for this user_view.   The
...
``SavingsAccount`` will be present::
    ...
    FROM "user_view" JOIN account ON "user_view".id = account.user_id
...
would use an outer join::
    ...
    SELECT "user_view".id AS user_id, "user_view".name AS user_name,
    account.balance AS account_balance
    FROM "user_view" LEFT OUTER JOIN account ON "user_view".id = account.user_id
...
a correlated SELECT::
    ...
    FROM account
    WHERE account.user_id = "user_view".id) > :param_1
