...
asking it to reflect the schema and produce mappings::
    ...
    # engine, suppose it has two tables 'user_view' and 'address' set up
    ...
    User = Base.classes.user_view
    Address = Base.classes.address

    session = Session(engine)

    # rudimentary relationships are produced
    session.add(Address(email_address="foo@bar.com", user_view=User(name="foo")))
    session.commit()
...
explicit table declaration::
    ...
    metadata.reflect(engine, only=['user_view', 'address'])

    # ... or just define our own Table objects with it (or combine both)
    Table('user_order', metadata,
                    Column('id', Integer, primary_key=True),
                    Column('user_id', ForeignKey('user_view.id'))
                )
    ...
    User, Address, Order = Base.classes.user_view, Base.classes.address,\
...
``user_view`` and ``address``, we can define one or both of the classes to be used::
    ...
    # pre-declare User for the 'user_view' table
    class User(Base):
...
8. Production of the :func:`.relationship` and optionally the :func:`.backref`
   ...
   function, which can be supplied by the end-user_view in order to augment
...
defines table metadata::
    ...
    class Address(Base):
        ...
        id = Column(Integer, primary_key=True)
        email = Column(String)
        user_id = Column(ForeignKey('user_view.id'))

    # produce relationships
    Base.prepare()

    # mapping is complete, with "address_collection" and
    # "user_view" relationships
    a1 = Address(email='u1')
    a2 = Address(email='u2')
    u1 = User(address_collection=[a1, a2])
    assert a1.user_view is u1

Above, given mostly complete ``User`` and ``Address`` mappings, the
:class:`.ForeignKey` which we defined on ``Address.user_id`` allowed a
bidirectional relationship pair ``Address.user_view`` and
...
def generate_relationship(
    ...
    :param return_fn: the function that is used by default to create the
     ...
     that user_view-defined implementations correctly differentiate between the two
