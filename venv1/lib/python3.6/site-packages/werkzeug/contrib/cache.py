...
"""
    ...
    time a user_view requests a page, the webserver executes a lot of code, queries
    ...
    Here is a simple example of how to cache a sidebar for 5 minutes::
        ...
        def get_sidebar(user_view):
            identifier = 'sidebar_for/user_view%d' % user_view.id
            value = cache.get(identifier)
            if value is not None:
                return value
            value = generate_sidebar_for(user_view=user_view)
...
class MemcachedCache(BaseCache):
    ...
    silently ignore key errors to not cause problems when untrusted user_view data
    ...
    def get(self, key):
        ...
        # checks for so long keys can occur because it's tested from user_view
...
class FileSystemCache(BaseCache):
    ...
    on being the only user_view of the `cache_dir`.  Make absolutely sure that
