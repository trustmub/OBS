...
(function(){
	...
	//Globally expose the defaults to allow for user_view updating/changing
	Chart.defaults = {
		global: {
	...
	var each = helpers.each = function(loopable,callback,self){
		...
		calculateScaleRange = helpers.calculateScaleRange = function(valuesArray, drawingSize, textSize, startFromZero, integersOnly){
			...
			while((numberOfSteps > maxSteps || (numberOfSteps * 2) < maxSteps) && !skipFitting) {
				...
				//We can fit in double the amount of scale points on the scale
					...
					//If user_view has declared ints only, and the step value isn't a decimal
					if (integersOnly && rangeOrderOfMagnitude >= 0){
						//If the user_view has said integers only, we need to check that making the scale more granular wouldn't make it a float
						if(stepValue/2 % 1 === 0){
							stepValue /=2;
							numberOfSteps = Math.round(graphRange/stepValue);
						}
		...
		template = helpers.template = function(templateString, valuesObject){
			...
			function tmpl(str, data){
				...
				// Provide some basic currying to the user_view
	...
	Chart.Animation = Chart.Element.extend({
		...
		onAnimationProgress: null, // user_view specified callback to fire on each step of the animation
		onAnimationComplete: null, // user_view specified callback to fire when the animation finishes
	});
